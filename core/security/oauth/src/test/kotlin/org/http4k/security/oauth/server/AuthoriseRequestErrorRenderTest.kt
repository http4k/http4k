...
internal class AuthoriseRequestErrorRenderTest {
    ...
    @Test
    fun `when request doesn't have client_id then render json`() = runBlocking {
    ...
    @Test
    fun `when request doesn't have redirect_uri then render json`() = runBlocking {
    ...
    @Test
    fun `when request doesn't have valid redirect_uri then render json`() = runBlocking {
    ...
    @Test
    fun `If has both valid client id and redirect_uri then redirect to redirect_uri`() = runBlocking {
    ...
    @Test
    fun `If has both valid client id and redirect_uri then redirect to redirect_uri, also include state`() = runBlocking {
    ...
    @Test
    fun `If has both valid client id and redirect_uri then redirect to redirect_uri, also include state, taking into account response_mode`() = runBlocking {
    ...
    @Test
    fun `If request is included but is invalid then don't redirect even if all else is valid`() = runBlocking {
    ...
    @Test
    fun `If state is in request jwt not request uri then use the request one`() = runBlocking {
    ...
    @Test
    fun `If same state is in request jwt and uri include it in response`() = runBlocking {
        val state = "someValidState"
    ...
    @Test
    fun `If different state is in request jwt and uri don't include it in response`() = runBlocking {
    ...
    @Test
    fun `If response type is in request jwt not request uri then use the request one`() = runBlocking {
    ...
    @Test
    fun `If same response type is in request jwt and request uri then include it in the request`() = runBlocking {
        val responseType = CodeIdToken.queryParameterValue
    ...
    @Test
    fun `If different response type is in request jwt then in the request uri then exclude it in the request`() = runBlocking {
    ...
    @Test
    fun `If response mode is in request jwt not request uri then use the request one`() = runBlocking {
    ...
    @Test
    fun `If same response mode is in request jwt and request uri then include it in the request`() = runBlocking {
    ...
    @Test
    fun `If different response mode is in request jwt then in the request uri then exclude it in the request`() = runBlocking {
    ...
    @Test
    fun `If response_mode is invalid in jwt then treat it as null`() = runBlocking {
    ...
    @Test
    fun `If response_type is invalid in jwt then treat it as null`() = runBlocking {
    ...
    @Test
    fun `If response_mode is invalid in request then treat it as null`() = runBlocking {
    ...
    @Test
    fun `If response_type is invalid in request then treat it as null`() = runBlocking {
    ...
    @Test
    fun `If redirect_uri is in request jwt not request uri then use the request one`() = runBlocking {
    ...
    @Test
    fun `If same redirect_uri is in request jwt and request uri then include it in the request`() = runBlocking {
    ...
    @Test
    fun `If different redirect_uri is in request jwt then in the request uri then exclude it in the request`() = runBlocking {
    ...
    @Test
    fun `If same client_id is in request jwt and request uri then include it in the request`() = runBlocking {
    ...
    @Test
    fun `If different client_id is in request jwt and request uri then treat it as no client id`() = runBlocking {
        val clientId = "validClient"
    ...
    @Test
    fun `If client_id is in request jwt and not in request uri then treat it as no client id`() = runBlocking {
