    ...
    private class CacheExecutor(
        ...
        fun execute(next: HttpHandler, request: Request): Response = when {
            ...
            request.directives().noStore -> next(request)
                ...
                if (cached == null) onMiss(next, request, now) else decideAndServe(cached, request, next, now)
            }
        }
            ...
            return when {
                vary.trim() == "*" -> false
                    ...
                    names.isEmpty() || names.all { name ->
                        request.headerValues(name).normalizedValues() == requestHeaders.valuesFor(name)
                            .normalizedValues()
                    }
                }
            }
        ...
        private fun CachedResponse.servableWhenError(now: Instant, request: Request): Boolean {
            ...
            return res.staleIfError?.let { staleFor <= it } == true || maxStaleAllows(request.directives(), staleFor)
        }
        ...
        private fun shouldStore(request: Request, response: Response): Boolean {
            ...
            request.header("Authorization") ?: return true
            val res = response.directives()
