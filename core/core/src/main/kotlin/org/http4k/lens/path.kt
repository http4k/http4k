...
fun Path.regex(pattern: String) = map(StringBiDiMappings.regex(pattern))
fun Path.regexGroup(pattern: String, group: Int = 1) = map(StringBiDiMappings.regexGroup(pattern, group))
fun Path.regexObject() = map(StringBiDiMappings.regexObject())
fun Path.urlEncoded() = map(StringBiDiMappings.urlEncoded())
fun Path.period() = map(StringBiDiMappings.period())
