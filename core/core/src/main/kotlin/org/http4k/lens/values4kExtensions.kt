 ...
 */
fun <VALUE : Value<T>, T : Any> BiDiPathLensSpec<String>.value(vf: ValueFactory<VALUE, T>) =
    map(vf::parse, vf::show)

/**
 * Custom lens for mapping to values4k Value
 */
fun <M : Any, VALUE : Value<T>, T : Any> BiDiLensSpec<M, String>.value(vf: ValueFactory<VALUE, T>) =
    map(vf::parse, vf::show)
