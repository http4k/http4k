...
object ServerForClientContract : HttpHandler {
    ...
    private val app = runBlocking {
        routes(
            ...
            "/echo" bind DELETE to { _: Request -> Response(OK).body("delete") },
            "/echo" bind GET to { request: Request -> Response(OK).body(request.uri.toString()) },
            ...
            "/hostheaders" bind { request: Request ->
                Response(OK).body(
                    request.headerValues("host").joinToString(",")
                )
            },
            ...
            "/multiRequestHeader" bind POST to { r: Request ->
                Response(OK).body(
                    r.headers.filter { it.first == "echo" }.map { it.first + ": " + it.second }.sorted()
                        .joinToString("\n")
                )
            },
