...
class ResponseCacheExtTest {
    ...
    @Test
    fun `adds public to response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `adds private to response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `adds no-cache to response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `adds only-if-cached to response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `adds must-revalidate to response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `adds no-store to response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `adds immutable to a response Cache-Control header`() = runBlocking {
    ...
    @Test
    fun `retrieves null max age if no Cache-Control header`() = runBlocking {
        val response = Response(OK)
        assertThat(response.maxAge(), equalTo(null))
    }

    @Test
    fun `retrieves null max age if Cache-Control header has no max-age directive`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "public")
        assertThat(response.maxAge(), equalTo(null))
    }

    @Test
    fun `retrieves null max age if Cache-Control header has invalid max-age directive`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "max-age=not_a_number")
        assertThat(response.maxAge(), equalTo(null))
    }

    @Test
    fun `retrieves max age from Cache-Control header if valid`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "public, max-age=60, stale-if-error=1000")
        assertThat(response.maxAge(), equalTo(60))
    }

    @Test
    fun `adds max-age to response Cache-Control header from Java duration`() = runBlocking {
    ...
    @Test
    fun `adds max-age to response Cache-Control header from Kotlin duration`() = runBlocking {
    ...
    @Test
    fun `retrieves null stale-while-revalidate if no Cache-Control header`() = runBlocking {
        val response = Response(OK)
        assertThat(response.staleWhileRevalidate(), equalTo(null))
    }

    @Test
    fun `retrieves null stale-while-revalidate if Cache-Control header has no directive`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "public")
        assertThat(response.staleWhileRevalidate(), equalTo(null))
    }

    @Test
    fun `retrieves null stale-while-revalidate if Cache-Control header has invalid directive`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "stale-while-revalidate=not_a_number")
        assertThat(response.staleWhileRevalidate(), equalTo(null))
    }

    @Test
    fun `retrieves stale-while-revalidate from Cache-Control header if valid`() = runBlocking {
    ...
    @Test
    fun `adds stale-while-revalidate to response Cache-Control header from Java duration`() = runBlocking {
    ...
    @Test
    fun `adds stale-while-revalidate to response Cache-Control header from Kotlin Duration`() = runBlocking {
    ...
    @Test
    fun `retrieves null stale-if-error if no Cache-Control header`() = runBlocking {
        val response = Response(OK)
        assertThat(response.staleIfError(), equalTo(null))
    }

    @Test
    fun `retrieves null stale-if-error if Cache-Control header has no directive`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "public")
        assertThat(response.staleIfError(), equalTo(null))
    }

    @Test
    fun `retrieves null stale-if-error if Cache-Control header has invalid directive`() = runBlocking {
        val response = Response(OK).header("Cache-Control", "stale-if-error=not_a_number")
        assertThat(response.staleIfError(), equalTo(null))
    }

    @Test
    fun `retrieves stale-if-error from Cache-Control header if valid`() = runBlocking {
    ...
    @Test
    fun `adds stale-if-error to response Cache-Control header from Java duration`() = runBlocking {
    ...
    @Test
    fun `adds stale-if-error to response Cache-Control header from Kotlin Duration`() = runBlocking {
    ...
    @Test
    fun `can chain together multiple calls to add to the header`() = runBlocking {
    ...
    @Test
    fun `should overwrite existing headers with new values`() = runBlocking {
