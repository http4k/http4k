...
class DiskStorage<T : Any>(
    ...
    override fun get(key: String): T? =
        resolveKey(key)?.takeIf { it.exists() }?.readText()?.let { autoMarshalling.asA(it, target) }
...
inline fun <reified T : Any> Storage.Companion.Disk(
    dir: File,
    autoMarshalling: AutoMarshalling = Moshi
): Storage<T> = DiskStorage(dir, T::class, autoMarshalling)
