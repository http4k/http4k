...
class Attribute<FINAL>(
    ...
    companion object {
        ...
        fun int() = base(N).map({ it.N!!.toInt() }, AttributeValue::Num)
        ...
        fun long() = base(N).map({ it.N!!.toLong() }, AttributeValue::Num)
        fun longs() = base(NS).map({ it.NS!!.map(String::toLong).toSet() }, AttributeValue::NumSet)
        fun double() = base(N).map({ it.N!!.toDouble() }, AttributeValue::Num)
        fun doubles() = base(NS).map({ it.NS!!.map(String::toDouble).toSet() }, AttributeValue::NumSet)
        fun float() = base(N).map({ it.N!!.toFloat() }, AttributeValue::Num)
        ...
        fun bigDecimal() = base(N).map({ it.N!!.toBigDecimal() }, AttributeValue::Num)
        fun bigDecimals() = base(NS).map({ it.NS!!.map(String::toBigDecimal).toSet() }, AttributeValue::NumSet)
        fun bigInteger() = base(N).map({ it.N!!.toBigInteger() }, AttributeValue::Num)
