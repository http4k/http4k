...
fun AwsRestJsonFake.SendEmail(messagesBySender: Storage<List<EmailMessage>>) = route<SendEmail> { data ->
    val source = data.fromEmailAddress!! // TODO proper error handling for missing value
    val existing = messagesBySender[source.value] ?: emptyList()

    val emailMessage = EmailMessage(
        source = source,
        destination = data.destination!!, // TODO proper error handling
