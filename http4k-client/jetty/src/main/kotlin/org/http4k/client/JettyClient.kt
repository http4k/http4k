...
object JettyClient {
    ...
    operator fun invoke(
        ...
        return JettyHandler(client, bodyMode, requestModifier)
    }

    private class JettyHandler(
        private val client: HttpClient,
        private val bodyMode: BodyMode,
        private val requestModifier: (JettyRequest) -> JettyRequest
    ) : DualSyncAsyncHttpHandler {
        ...
        private fun JettyResponse.toHttp4kResponse() = Response(Status(status, reason)).headers(headers.toHttp4kHeaders())

        private fun HttpFields.toHttp4kHeaders(): Headers = flatMap { it.values.map { hValue -> it.name to hValue } }

        private fun Throwable.asHttp4kResponse() = Response(
            when (this) {
