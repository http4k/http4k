...
internal fun Resource.toAnthropic(): Source = when (this) {
...
internal fun ToolRequest.toAnthropic() = Content.ToolUse(name, ToolUseId.of(id.value), arguments)

internal fun org.http4k.ai.llm.model.Content.toAnthropic() = when (this) {
...
internal fun Content.toLLM() = when (this) {
...
internal fun ToolSelection.toLLM() = when (this) {
    Auto -> ToolChoice.Auto(true)
    Required -> ToolChoice.Auto(true)
}

internal fun LLMTool.toAnthropic() = Tool(name, description, inputSchema)
