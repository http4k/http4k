...
### Installation (Gradle)
```compile group: "org.http4k", name: "http4k-format-xml", version: "3.119.0"```
```compile group: "org.http4k", name: "http4k-format-jackson", version: "3.119.0"```

### About
These modules provide auto-marshalling functionality to convert XML into arbitrary data classes. The 2 differ slightly in 
their behaviour:

### JacksonXml
- Provides bi-directional conversion
- Does NOT expose an XML DOM node model
- Generates a wrapper element to represent the parent node
- Correctly handles repeating child-elements
- Provides extension point to map custom types using BiDiMapping registration
...
- Has trouble with repeating child-elements, depending on zero, one or many elements in the XML. This is due to the underlying 
library used for the implementation.
