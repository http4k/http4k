...
sealed class HttpEvent(
    ...
    class Incoming(
        ...
        constructor(tx: HttpTransaction) : this(
            ...
            (tx.request as? RoutedRequest)?.xUriTemplate?.toString() ?: tx.request.uri.path.trimStart('/')
        )
    ...
    class Outgoing(
        ...
        constructor(tx: HttpTransaction) : this(
            ...
            tx.response.status,
            tx.duration.toMillis(),
            (tx.response as? RoutedResponse)?.xUriTemplate?.toString() ?: tx.request.uri.path.trimStart('/')
        )
