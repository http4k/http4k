...
typealias RequestMatcher = (Request) -> RouterMatch

fun Boolean.asRouterMatch() = if(this) RouterMatch.Matched else Unmatched

/**
 * For routes where certain queries are required for correct operation. RequestMatch is composable.
 */
fun queries(vararg names: String): RequestMatcher = { req -> names.all { req.query(it) != null }.asRouterMatch() }

/**
 * For routes where certain headers are required for correct operation. RequestMatch is composable.
 */
fun headers(vararg names: String): RequestMatcher = { req -> names.all { req.header(it) != null }.asRouterMatch() }
...
fun hostDemux(head: Pair<String, RoutingHttpHandler>, vararg tail: Pair<String, RoutingHttpHandler>): RoutingHttpHandler {
    ...
    return routes(*hostHandlerPairs.map { { req: Request -> (req.header("host") == it.first).asRouterMatch() } bind it.second }.toTypedArray())
}
...
infix fun RequestMatcher.and(that: RequestMatcher): RequestMatcher = { val initial: RouterMatch = RouterMatch.Matched
    listOf(this, that).fold(initial) { acc, next -> acc.and(next(it)) } }

internal class PredicatedHandler(private val predicate: (Request) -> RouterMatch, private val handler: HttpHandler) : RoutingHttpHandler {
    ...
    override fun match(request: Request) = if (predicate(request) == RouterMatch.Matched) MatchingHandler(handler) else Unmatched
...
internal data class RequestMatchRoutingHttpHandler(
    private val matched: (Request) -> RouterMatch,
    ...
    override fun match(request: Request) = if (matched(request) != RouterMatch.Matched) Unmatched else httpHandler.match(request)
