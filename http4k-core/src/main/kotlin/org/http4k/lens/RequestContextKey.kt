...
object RequestContextKey {
    fun <T> required(contexts: RequestContexts, name: String = UUID.randomUUID().toString()): RequestContextLens<T> {
    ...
    fun <T : Any?> optional(contexts: RequestContexts, name: String = UUID.randomUUID().toString()) =
        BiDiLens(Meta(false, "context", ObjectParam, name), { target -> contexts[target][name] },
            { value: T?, target: Request -> contexts[target][name] = value; target }
        )

    fun <T : Any?> defaulted(contexts: RequestContexts, default: T, name: String = UUID.randomUUID().toString()) =
