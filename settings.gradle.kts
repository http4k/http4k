rootProject.name = "http4k"
include("http4k-core")
include("http4k-aws")
include("http4k-bom")
include("http4k-client-apache")
include("http4k-client-apache4")
include("http4k-client-apache-async")
include("http4k-client-apache4-async")
include("http4k-client-jetty")
include("http4k-client-okhttp")
include("http4k-client-websocket")
include("http4k-cloudnative")
include("http4k-contract")
include("http4k-format-argo")
include("http4k-format-gson")
include("http4k-format-jackson")
include("http4k-format-jackson-xml")
include("http4k-format-jackson-yaml")
include("http4k-format-kotlinx-serialization")
include("http4k-format-moshi")
include("http4k-format-xml")
include("http4k-incubator")
include("http4k-jsonrpc")
include("http4k-metrics-micrometer")
include("http4k-multipart")
include("http4k-resilience4j")
include("http4k-opentelemetry")
include("http4k-security-oauth")
include("http4k-server-apache")
include("http4k-server-apache4")
include("http4k-server-jetty")
include("http4k-server-ktorcio")
include("http4k-server-ktornetty")
include("http4k-server-netty")
include("http4k-server-ratpack")
include("http4k-server-undertow")
include("http4k-serverless-alibaba")
include("http4k-serverless-alibaba:integration-test:test-function")
include("http4k-serverless-azure")
include("http4k-serverless-azure:integration-test:test-function")
include("http4k-serverless-gcf")
include("http4k-serverless-gcf:integration-test:test-function")
include("http4k-serverless-lambda")
include("http4k-serverless-lambda:integration-test")
include("http4k-serverless-lambda:integration-test:test-function")
include("http4k-serverless-openwhisk")
include("http4k-serverless-openwhisk:integration-test")
include("http4k-serverless-openwhisk:integration-test:test-function")
include("http4k-serverless-tencent")
include("http4k-serverless-tencent:integration-test:test-function")
include("http4k-template-dust")
include("http4k-template-freemarker")
include("http4k-template-handlebars")
include("http4k-template-pebble")
include("http4k-template-thymeleaf")
include("http4k-template-jade4j")
include("http4k-testing-approval")
include("http4k-testing-chaos")
include("http4k-testing-hamkrest")
include("http4k-testing-kotest")
include("http4k-testing-servirtium")
include("http4k-testing-webdriver")
