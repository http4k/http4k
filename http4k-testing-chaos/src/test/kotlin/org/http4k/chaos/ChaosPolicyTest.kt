...
class ChaosPolicyTest {
    ...
    @Test
    fun `Until stops a behaviour when triggered2`() {
        val http = Always.inject(ReturnStatus(INTERNAL_SERVER_ERROR)).until { it.request.method == POST }
                .then(Always.inject(ReturnStatus(NOT_FOUND)))
                .asFilter().then { Response(OK) }

        http(Request(GET, "/foo")) shouldMatch hasStatus(INTERNAL_SERVER_ERROR).and(hasHeader("x-http4k-chaos", "Status 500"))
        http(Request(POST, "/bar")) shouldMatch hasStatus(NOT_FOUND).and(hasHeader("x-http4k-chaos", "Status 404"))
        http(Request(GET, "/bar")) shouldMatch hasStatus(NOT_FOUND).and(hasHeader("x-http4k-chaos", "Status 404"))
    }
}