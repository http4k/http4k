...
object CompletionDetailSpanModifiers : McpOpenTelemetrySpanModifier {
    override operator fun invoke(sb: Span, request: McpJsonRpcRequest) {
        if (request is McpCompletion.Request) {
            sb.setAttribute("gen_ai.completion.arguments", McpJson.asFormatString(request.params.argument))
        }
    }

    override operator fun invoke(sb: Span, response: McpJsonRpcResponse) {
        if (response is McpCompletion.Response) {
            sb.setAttribute("gen_ai.completion.result", McpJson.asFormatString(response.result.completion))
        }
    }
}
