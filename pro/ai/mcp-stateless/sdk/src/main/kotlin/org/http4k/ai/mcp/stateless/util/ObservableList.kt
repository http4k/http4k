...
import org.http4k.ai.mcp.stateless.server.protocol.ObservableCapability
...
open class ObservableList<T>(initial: Iterable<T>) : ObservableCapability<T>, Iterable<T> {
    ...
    protected fun <R> derived(fn: (Iterable<T>) -> R): () -> R {
        val cached = AtomicReference(fn(items))
        onChange(cached) { cached.set(fn(items)) }
        return cached::get
    }
}
