...
class McpTasks(
    ...
    private fun update(taskId: TaskId, status: TaskStatus, fn: (TaskRecord) -> TaskRecord) {
        store.computeIfPresent(taskId) { _, record ->
            when {
                record.task.status.isTerminal -> record
                else -> fn(record.moved(status))
            }
        }
    }

    private fun TaskRecord.moved(status: TaskStatus) = copy(
        task = task.copy(status = status, lastUpdatedAt = clock()),
        resume = resume.takeUnless { status.isTerminal },
        inputRequests = inputRequests.takeUnless { status.isTerminal },
    )
    ...
    private fun TaskRecord.expireIfStale() = when {
        ...
        else -> moved(failed).also { store[task.taskId] = it }
    }
