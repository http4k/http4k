...
import org.http4k.ai.mcp.stateless.protocol.messages.ResourceNotFoundError
...
private class InMemoryResources(
    ...
    private val updateSubscribers = ConcurrentHashMap<Any, Pair<Set<String>, (McpUri) -> Unit>>()

    override fun triggerUpdated(uri: McpUri) =
        updateSubscribers.values.forEach { (uris, handler) -> if (uri.value in uris) handler(uri) }

    override fun subscribeToUpdates(key: Any, uris: Set<String>, handler: (McpUri) -> Unit) {
