...
class CacheScopeTest {
    ...
    fun `resources-read result takes cacheScope from the capability, not the response`() {
        val cap = ResourceCapability(Resource.Static(McpUri.of("res://cfg"), ResourceName.of("cfg"), cacheScope = private)) {
            ResourceResponse.Ok(listOf(Resource.Content.Text("hi", McpUri.of("res://cfg"))))
        }

        val result = cap.read(McpResource.Read.Request.Params(McpUri.of("res://cfg")), NoOp, Request(GET, "/"))
