...
class McpTasksTest {
    ...
    private val store = ConcurrentHashMap<TaskId, TaskRecord>()
    private val tasks = McpTasks(store = store, clock = { now }, newTaskId = { taskId })
    ...
    @Test
    fun `a task awaiting input holds its resumption, a settled one lets it go`() {
        server(call("asks"))
        awaitStatus(taskId, "input_required")
        assertThat(store[taskId]!!.resume != null, equalTo(true))

        server(update(taskId))
        awaitStatus(taskId, "completed")

        assertThat(store[taskId]!!.resume, equalTo(null))
        assertThat(store[taskId]!!.inputRequests, equalTo(null))
    }

    @Test
    fun `a cancelled task lets go of its resumption`() {
        server(call("asks"))
        awaitStatus(taskId, "input_required")
