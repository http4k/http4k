...
import org.http4k.routing.stateless.mcpA2aBridgeServer
import org.http4k.testing.toHttpHandler
import org.junit.jupiter.api.Test
import java.util.Random
import org.http4k.ai.mcp.stateless.protocol.Version as McpVersion
...
class McpA2aBridgeServerTest {
    ...
    fun `MCP server forwards auth header to A2A agent`() {
        ...
        val addAuth = Filter { next -> { next(it.header("Authorization", "Bearer xyz")) } }
        addAuth.then(server.http!!).testMcpClient().use { client ->
            val response = client.tools().call(ToolName.of("send_message"), ToolRequest(args = mapOf("message" to "hi")))
