...
/**
 * Mcp-Name is stamped by toHttpRequest (see mirroredName); this promotes
 * x-mcp-header args to Mcp-Param-*.
 */
                ...
                ?.let { tool ->
                    tool.mcpHeaderAnnotations().fold(request) { req, param ->
                        arguments[param.first]?.let {
                            req.header("Mcp-Param-${param.second}", encodeMcpHeaderValue(it.toString()))
                        } ?: req
                    }
                } ?: request
        )
