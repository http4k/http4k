...
import org.http4k.ai.mcp.stateless.McpError.Http
...
import org.http4k.ai.mcp.stateless.client.internal.SubscriptionSpec
import org.http4k.ai.mcp.stateless.client.internal.asOrFailure
import org.http4k.ai.mcp.stateless.client.internal.domainErrorOr
import org.http4k.ai.mcp.stateless.client.internal.serverInfoOrNull
import org.http4k.ai.mcp.stateless.client.internal.toElicitationRequest
import org.http4k.ai.mcp.stateless.client.internal.toHttpRequest
import org.http4k.ai.mcp.stateless.client.internal.toToolResponseOrError
import org.http4k.ai.mcp.stateless.model.ElicitationAction.cancel
...
import org.http4k.ai.mcp.stateless.protocol.ClientProtocolCapability.ElicitationUrl
...
import org.http4k.ai.mcp.stateless.protocol.messages.McpLogging
import org.http4k.ai.mcp.stateless.protocol.messages.McpProgress
...
class HttpMcpClient(
    ...
    // declaring tasks licenses the server to answer any tools/call with a handle, so it is opt-in
    ...
    override fun prompts() = object : McpClient.Prompts {
        ...
        override fun get(
            ...
            return http
                ...
                .flatMapFailure { it.domainErrorOr { m -> PromptResponse.Error(m) } }
        }
    ...
    override fun resources() = object : McpClient.Resources {
        ...
        override fun read(
            ...
            return http
                ...
                .flatMapFailure { it.domainErrorOr { m -> ResourceResponse.Error(m) } }
        }

        override fun onListChanged(handler: () -> Unit) = listenFor { onResourcesChanged(handler) }
        override fun subscribe(uri: McpUri, handler: () -> Unit) = listenFor { onResourceUpdated(uri, handler) }
    }
    ...
    override fun completions() = object : McpClient.Completions {
        override fun complete(ref: Reference, request: CompletionRequest) =
            http.send<McpCompletion.Response.Result>(
                ...
                .flatMapFailure { it.domainErrorOr { m -> CompletionResponse.Error(m) } }
    }

    override fun tasks() = object : McpClient.Tasks {
        override fun get(taskId: TaskId) =
            http.send<McpTask.Result>(McpTask.Get.Request(McpTask.Get.Request.Params(taskId), nextId()))

        override fun update(taskId: TaskId, inputResponses: Map<String, ElicitationResponse>) =
            http.send<McpTask.Result>(
                McpTask.Update.Request(McpTask.Update.Request.Params(taskId, inputResponses.toWire()), nextId())
            )

        override fun cancel(taskId: TaskId) =
            http.send<McpTask.Result>(McpTask.Cancel.Request(McpTask.Cancel.Request.Params(taskId), nextId()))
    }
    ...
    private inline fun <reified T : Any> HttpHandler.send(
        ...
        return when {
            response.contentType()?.equalsIgnoringDirectives(TEXT_EVENT_STREAM) == true ->
