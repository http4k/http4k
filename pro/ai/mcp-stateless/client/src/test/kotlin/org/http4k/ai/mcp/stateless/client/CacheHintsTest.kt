...
class CacheHintsTest {
    ...
    private val resource = Resource.Static(McpUri.of("res://cfg"), ResourceName.of("cfg")) bind {
    ...
    fun `resources-read carries the handler's per-response ttl`() {
        val ok = client.resources().read(ResourceRequest(McpUri.of("res://cfg"))).valueOrNull() as ResourceResponse.Ok
