...
class PopulateToolHeadersTest {
    ...
    fun `adds Mcp-Param headers for annotated properties`() {
        ...
        val filter = headersFor(listOf(toolWithHeaders), "execute_sql", mapOf("region" to "us-west1", "priority" to 42, "query" to "SELECT 1"))
    ...
    fun `omits header when argument is absent`() {
        var captured: Request? = null
        val filter = headersFor(listOf(toolWithHeaders), "execute_sql", mapOf("query" to "SELECT 1"))
        filter { captured = it; Response(OK) }(Request(POST, "/mcp"))
    ...
    fun `no extra headers when tool has no annotations`() {
        var captured: Request? = null
        val filter = headersFor(listOf(toolWithoutHeaders), "simple_tool", mapOf("input" to "hello"))
        filter { captured = it; Response(OK) }(Request(POST, "/mcp"))
    ...
    fun `no extra headers when tool not found in list`() {
        var captured: Request? = null
        val filter = headersFor(emptyList(), "unknown", mapOf("region" to "us-west1"))
        filter { captured = it; Response(OK) }(Request(POST, "/mcp"))
    ...
    fun `boolean values encoded as strings`() {
        ...
        var captured: Request? = null
        val filter = headersFor(listOf(tool), "bool_tool", mapOf("verbose" to true))
        filter { captured = it; Response(OK) }(Request(POST, "/mcp"))
    ...
    @Test
    fun `a value a header cannot carry is wrapped in the base64 sentinel`() {
        var captured: Request? = null
        val filter = headersFor(listOf(toolWithHeaders), "execute_sql", mapOf("region" to "caf\u00e9"))
        filter { captured = it; Response(OK) }(Request(POST, "/mcp"))

        assertThat(captured!!.header("Mcp-Param-Region"), equalTo("=?base64?Y2Fmw6k=?="))
    }

    @Test
    fun `a value a header can carry is sent as a literal`() {
        var captured: Request? = null
        val filter = headersFor(listOf(toolWithHeaders), "execute_sql", mapOf("region" to "us-west1"))
        filter { captured = it; Response(OK) }(Request(POST, "/mcp"))

        assertThat(captured!!.header("Mcp-Param-Region"), equalTo("us-west1"))
    }

    private fun headersFor(tools: List<McpTool>, name: String, arguments: Map<String, Any>) =
        PopulateToolHeaders(tools.headerParamsByName()[ToolName.of(name)].orEmpty(), arguments)
}
