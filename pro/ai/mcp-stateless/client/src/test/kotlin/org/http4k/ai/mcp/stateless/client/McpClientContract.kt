...
import org.http4k.ai.mcp.stateless.coerce
import org.http4k.ai.mcp.stateless.model.CompletionArgument
...
import org.http4k.ai.mcp.stateless.util.McpJson.auto
import org.http4k.ai.mcp.stateless.util.McpJson.obj
import org.http4k.ai.mcp.stateless.util.McpJson.string
import org.http4k.ai.model.Role.Companion.Assistant
import org.http4k.ai.model.ToolName
import org.http4k.lens.stateless.with
...
abstract class McpClientContract : PortBasedTest {
    ...
    fun `can list and read resources`() {
        val resources = resources(
            Resource.Static(McpUri.of("https://http4k.org"), ResourceName.of("HTTP4K"), "description") bind {
                ResourceResponse.Ok(listOf(Resource.Content.Text("foo", McpUri.of("https://http4k.org"))))
            },
        ...
        withMcpServer(resources = resources) {
            ...
            assertThat(
                resources().read(ResourceRequest(McpUri.of("https://http4k.org"))).coerce<ResourceResponse.Ok>(),
                equalTo(ResourceResponse.Ok(listOf(Resource.Content.Text("foo", McpUri.of("https://http4k.org")))))
            )
        }
    }
    ...
    fun `can complete references`() {
        val completions = completions(
            Reference.ResourceTemplate(McpUri.of("https://http4k.org")) bind { CompletionResponse.Ok(listOf("1", "2")) }
        )

        withMcpServer(completions = completions) {
            assertThat(
                completions().complete(
                    Reference.ResourceTemplate(McpUri.of("https://http4k.org")),
    ...
    fun `drives a resources-read input-required round-trip`() {
        val resources = resources(
            Resource.Static(McpUri.of("res://greet"), ResourceName.of("greet")) bind { req ->
                when (req.inputResponses["login"]) {
        ...
        withMcpServer(resources = resources) {
            ...
            val first = resources().read(ResourceRequest(McpUri.of(uri))).valueOrNull() as ResourceResponse.InputRequired
            assertThat(first.requestState, equalTo("s3"))

            val retried = resources().read(
                ResourceRequest(McpUri.of(uri), inputResponses = login, requestState = first.requestState)
            )
            assertThat(
                retried.valueOrNull() as ResourceResponse.Ok,
                equalTo(ResourceResponse.Ok(listOf(Resource.Content.Text("hi [state=s3]", McpUri.of(uri)))))
            )
        }
    }
    ...
    fun `a resources list change fires the onListChanged handler`() {
        assumeTrue(streamsSubscriptions)
        val resources = resources(
            Resource.Static(McpUri.of("res://r"), ResourceName.of("r"), "d") bind {
                ResourceResponse.Ok(listOf(Resource.Content.Text("x", McpUri.of("res://r"))))
            }
        )
    ...
    fun `a resource update fires the subscribe handler for that URI only`() {
        assumeTrue(streamsSubscriptions)
        val resources = resources(
            Resource.Static(McpUri.of("res://watched"), ResourceName.of("w"), "d") bind {
                ResourceResponse.Ok(listOf(Resource.Content.Text("x", McpUri.of("res://watched"))))
            }
        )

        withMcpServer(resources = resources) {
            val watched = CountDownLatch(1)
            val sub = resources().subscribe(McpUri.of("res://watched")) { watched.countDown() }.valueOrNull()!!
            try {
                assertThat(
                    awaitFiring(watched) {
                        resources.triggerUpdated(McpUri.of("res://ignored")) // no handler for this URI
                        resources.triggerUpdated(McpUri.of("res://watched"))
                    },
                    equalTo(true)
                )
            } finally {
