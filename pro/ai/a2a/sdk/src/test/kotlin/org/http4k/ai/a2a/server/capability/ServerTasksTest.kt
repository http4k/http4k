...
class ServerTasksTest {
    ...
    fun `list returns empty page when no tasks exist`() {
        val response = tasks.list(A2ATask.List.Request.Params())
    ...
    fun `list filters by contextId`() {
        ...
        val response = tasks.list(A2ATask.List.Request.Params(contextId = ContextId.of("ctx-1")))
    ...
    fun `list filters by status`() {
        ...
        val response = tasks.list(A2ATask.List.Request.Params(status = TaskState.working))
    ...
    fun `list respects pageSize limit`() {
        ...
        val response = tasks.list(A2ATask.List.Request.Params(pageSize = 2))
    ...
    fun `list paginates using pageToken`() {
        ...
        val firstResponse = tasks.list(A2ATask.List.Request.Params(pageSize = 2))
        val secondResponse = tasks.list(A2ATask.List.Request.Params(pageSize = 2, pageToken = firstResponse.nextPageToken))
    ...
    fun `get returns response with task when found`() {
        ...
        val response = tasks.get(A2ATask.Get.Request.Params(id = TaskId.of("task-1")))
    ...
    fun `get returns null when task not found`() {
        val response = tasks.get(A2ATask.Get.Request.Params(id = TaskId.of("non-existent")))
    ...
    fun `cancel returns response with cancelled task when found`() {
        ...
        val response = tasks.cancel(A2ATask.Cancel.Request.Params(id = TaskId.of("task-1")))
    ...
    fun `cancel returns null when task not found`() {
        val response = tasks.cancel(A2ATask.Cancel.Request.Params(id = TaskId.of("non-existent")))
    ...
    fun `list returns all tasks without pagination when pageSize is null`() {
        ...
        val response = tasks.list(A2ATask.List.Request.Params(pageSize = null))
