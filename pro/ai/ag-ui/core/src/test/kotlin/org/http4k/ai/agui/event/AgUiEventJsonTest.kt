...
import org.http4k.ai.agui.model.ErrorCode
...
class AgUiEventJsonTest {
    ...
    fun `RunError roundtrips polymorphically`() {
        val event: AgUiEvent = RunError(message = "boom", code = ErrorCode.of("INTERNAL"))
        val json = AgUiJson.asFormatString(event)
    ...
    fun `Reasoning events roundtrip`() {
        val events: List<AgUiEvent> = listOf(
            ...
            ReasoningEncryptedValue(subtype = "message", entityId = EntityId.of("e"), encryptedValue = EncryptedValue.of("x=="))
        )
    ...
    fun `Raw and Custom events roundtrip`() {
        val raw: AgUiEvent = Raw(event = AgUiJson.parse("""{"foreign":1}"""), source = RawEventSource.of("external"))
        val custom: AgUiEvent = Custom(name = CustomEventName.of("ping"), value = AgUiJson.parse("\"pong\""))
