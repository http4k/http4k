...
class ExportHarTest : HttpWiretapFunctionContract, McpWiretapFunctionContract {

    override val toolName = "export_har"

    private val store = TransactionStore.InMemory()

    override val function = ExportHar(store)
    ...
    fun `http exports HAR for transaction`(approver: Approver) {
        val tx = record()
        approver.assertApproved(httpClient()(Request(GET, "/${tx.id}/har")))
    }

    @Test
    fun `http returns 404 for unknown id`() {
        val response = httpClient()(Request(GET, "/999/har"))
        assertThat(response.status, equalTo(NOT_FOUND))
    }

    @Test
    fun `mcp exports HAR for transaction`(approver: Approver) {
        val tx = record()
