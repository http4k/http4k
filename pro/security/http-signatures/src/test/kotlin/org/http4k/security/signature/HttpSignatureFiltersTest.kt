...
class HttpSignatureFiltersTest {
    ...
    fun `client filter adds signature headers to request`() {
        val signingFilter = ClientFilters.SignHttpRequest(listOf(Method, Path), keyId, keyPair.private, RsaPssSha512)
    ...
    fun `server filter rejects requests with missing required components`() {
        ...
        val signingFilter = ClientFilters.SignHttpRequest(listOf(Method), keyId, keyPair.private, RsaPssSha512)
    ...
    fun `server filter accepts requests with all required components`() {
        ...
        val signingFilter = ClientFilters.SignHttpRequest(components, keyId, keyPair.private, RsaPssSha512)
