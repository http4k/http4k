...
class SignatureTest {
    ...
    fun `can parse signature input header for a request`() {
        ...
        val input = SignatureInputParser(SignatureComponentFactory.HttpRequest())(Request(GET, "").header("Signature-Input", headerValue))!!
    ...
    fun `can parse signature input header for a response`() {
        ...
        val input = SignatureInputParser(SignatureComponentFactory.HttpResponse())(Response(OK).header("Signature-Input", headerValue))!!

        assertThat(input, equalTo(
            listOf(
                SignatureInput("sig1",
                    listOf(SignatureComponent.Status, SignatureComponent.Header("content-type")),
                    SignatureParameters("test-key-rsa", "rsa-pss-sha512", Instant.ofEpochSecond(1618884473L), null, null, null))
            )
        ))
    }
    ...
    fun `comprehensive test demonstrating all components`() {
        ...
        val signatureBaseResult = SignatureBaseCreator.Default<Request>()
            .invoke(request, allComponents, signatureParams)
