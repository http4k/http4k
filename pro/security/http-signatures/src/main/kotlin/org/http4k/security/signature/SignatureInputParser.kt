 ...
 */
class SignatureInputParser<M : HttpMessage>(private val componentsFactory: SignatureComponentFactory<M>) :
    LensExtractor<M, List<SignatureInput<M>>?> {
    override operator fun invoke(target: M) = target.header("Signature-Input")?.let {
    ...
    private fun parseSignatureInputValue(label: String, value: String): SignatureInput<M>? = when {
    ...
    private fun parseComponentsList(componentsString: String): List<SignatureComponent<M>> =
        when {
            componentsString.isEmpty() -> emptyList()
            else -> {
                val components = mutableListOf<SignatureComponent<M>>()
                ...
                if (lastComp.isNotEmpty()) parseComponent(lastComp).let { components.add(it) }

                components
            }
        }

    private fun parseComponent(componentString: String): SignatureComponent<M> {
        ...
        return componentsFactory(name, params) ?: SignatureComponent.Header(name, params)
    }
