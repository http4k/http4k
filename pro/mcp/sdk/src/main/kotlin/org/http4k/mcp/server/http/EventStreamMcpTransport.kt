...
class EventStreamMcpTransport(
    ...
    override fun receive(sId: SessionId, request: Request): Response {
        if (!sessionProvider.verify(sId, request)) return error()
        protocol.handleInitialize(
            McpInitialize.Request(VersionedMcpEntity(McpEntity.of("server"), Version.of("1")),
                ClientCapabilities()
            ),
            sId,
            this
        ...
        return protocol.receive(sId, request, this)
    }
