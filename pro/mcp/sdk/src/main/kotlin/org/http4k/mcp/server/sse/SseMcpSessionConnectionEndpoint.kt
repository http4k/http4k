 ...
 */
fun SseMcpSessionConnectionEndpoint(transport: SseMcpTransport) = sse {
    it.send(
        SseMessage.Event(
            "endpoint",
            Request(GET, "/message").with(sessionId of transport.newSession(it.connectRequest, it)).uri.toString()
        )
    )
}
